...
async function getNewTweets(authorization, userId, keyword) {
  ...
  if (exist) {
    ...
    if (nonExist.length) {
      ...
      return keyword? nonExist.filter((item) =>
      ...
      const tweetsData = tweets.map(({ tweetId, content, created_at }) => {
        return {
          ...
          content: `#twitter #new_tweet"\n\n🔊 New Tweet 🔊\n\n<a href='https://twitter.com/${
            profile
          }/status/${tweetId}'>${profile}</a> - ${formatDate(
            created_at
          )}\n\n${content}`,
        };
      });
...
async function startBot() {
  ...
  const validProfiles = profiles.filter(Boolean).filter(
    (value, index) =>
...
(function () {
  ...
  const local = localStorage.getItem("twitter-bot-storage2")
    ? JSON.parse(localStorage.getItem("twitter-bot-storage2"))
    : {
        authorization: "",
        interval: 60,
        profiles: ""
  ...
  const { authorization, interval, profiles } = local;
